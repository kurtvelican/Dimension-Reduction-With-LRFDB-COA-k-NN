function [GlobalMin,GlobalParams] = lrfdb_coa(egitimSeti, testSeti, egitimSinifVerisi, testSinifVerisi, komsu_sayisi)

D = size(egitimSeti, 2);

% n_coy = 5, n_packs = 20, lb = 0, ub = 1
rand('state', sum(100*clock));
[n_coy, n_packs, nfevalMAX, lb, ub] = problem_terminate(D);
lu=[lb;ub];
VarMin = lu(1,:);
VarMax = lu(2,:);
p_leave = 0.005*n_coy^2;
Ps = 1/D;
pop_total   = n_packs*n_coy; % 100
costs       = zeros(pop_total,1); % (100,1) matrix
...
for c=1:pop_total
    costs(c,1) = knnFunc(coyotes(c,:),egitimSeti, testSeti, egitimSinifVerisi, testSinifVerisi,komsu_sayisi);
end
nfeval = 0;
...
while nfeval<nfevalMAX % Stopping criteria
    ...
    for p=1:n_packs
        ...
        for c=1:n_coy_aux
            ...
            new_cost = knnFunc(new_coyotes(c,:),egitimSeti, testSeti, egitimSinifVerisi, testSinifVerisi,komsu_sayisi);
            nfeval   = nfeval+1;
        ...
        pup_cost    = knnFunc(pup,egitimSeti, testSeti, egitimSinifVerisi, testSinifVerisi,komsu_sayisi);
        nfeval      = nfeval + 1;
